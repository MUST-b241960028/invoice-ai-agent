...
Энэ repo нь GitHub Actions workflow-той:

- Python dependency суулгана
- `app.py`, `pipeline.py` compile шалгана
- demo data болон SQLite DB байгаа эсэхийг шалгана
- Docker image build хийж deploy-ready эсэхийг баталгаажуулна

GitHub дээр push хийсний дараа **Actions** tab-аас workflow status харна.

Streamlit Cloud дээр deploy хийх бол:

1. Repo-г Streamlit Cloud-д холбоно
2. Main file path: `app.py`
3. Secrets хэсэгт `ANTHROPIC_API_KEY` нэмнэ

Docker ашиглаж deploy хийх бол:

```bash
docker build -t smart-invoice-agent .
docker run -p 8501:8501 -e ANTHROPIC_API_KEY="sk-ant-..." smart-invoice-agent
```

## Eval Agent dataset ажиллуулах

`dataset/eval_agent/eval_agent/` доторх 100 invoice дээр agent-ийг ажиллуулж шалгах:

```bash
export ANTHROPIC_API_KEY="sk-ant-..."
python evaluate_eval_agent.py
```

Үр дүн нь `eval_agent_results.json` файлд хадгалагдана.

GitHub дээр **Actions → Evaluate Agent → Run workflow** гэж ажиллуулж болно. Үүний өмнө repo secrets-д `ANTHROPIC_API_KEY` нэмэх шаардлагатай.
